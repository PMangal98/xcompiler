#ifndef ASS5_16CS10041_TRANSLATOR_H
#define ASS5_16CS10041_TRANSLATOR_H
...
class type_n;			// Types and the list
class expresn;			// Storing expression as truelist,falselist and nextlist
class quad; 			// Single quad
class symdata; 			// Data for symbol table
class symtab; 			// Symbol table
class quad_arr; 		// Quad array that also emits the required emits
class funct;
class array;
...
extern int glob_width; 		// Global width for inherited attributes
extern int next_instr; 		// Next instruction for use in quads and in different function like backpatch
extern int temp_count; 		// count of the temporary varibles to name the new temporary variable
extern symtab *glob_st; 	// Pointer to the Global Symbol Table
...
/**
 * Single entry for the symbol table
 */ 
class symdata{
public:
...
public:
	...
	opcode op;
	void print_arg();
	quad(opcode,string,string,string); 	//constructor definition
};
...
struct idStr
{
	symdata *loc;			// pointer to the Symbol Table fot this identifier
	string *name;			// Name of the identifier (stored as a string)
};
...
public:
	...
	quad_arr();
	void emit(opcode opc, string arg1="", string arg2="", string result=""); 	// emit for 3 inputs
